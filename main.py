...
def get_text_messages(message):
    ...
    if message.text == BUTTON_USD:
        ...
        values = Data(site)
        BOT.send_message(message.chat.id, 'Что вас интересует?', reply_markup=KEYBOARD_CHOISE)
        ...
        BOT.send_message(message.chat.id, values.get_all_list(), reply_markup=KEYBOARD_CHOISE)

    elif message.text == BUTTON_BEST_BUY_VALUE:
        BOT.send_message(message.chat.id, values.get_best_buy_value(), reply_markup=KEYBOARD_CHOISE)

    elif message.text == BUTTON_BEST_SELL_VALUE:
        BOT.send_message(message.chat.id, values.get_best_sell_value(), reply_markup=KEYBOARD_CHOISE)
